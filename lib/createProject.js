...
async function createProject(projectName) {
  try {
    ...
    try {
      ...
      const templateDir = templateMap[options.template]
      if (!templateDir) {
        spinner.fail('不支持的模板类型')
        console.error(chalk.red(`不支持的模板: ${options.template}`))
        process.exit(1)
      }
  ...
  try {
    ...
    if (options.testFramework === 'vitest') {
      ...
      if (options.template === 'react') {
        ...
        packageJson.devDependencies['@testing-library/vue'] = '^8.0.1'
        packageJson.devDependencies['@testing-library/jest-dom'] = '^6.1.5'
      }
      ...
      if (options.template === 'react') {
        ...
        packageJson.devDependencies['@testing-library/vue'] = '^8.0.1'
        packageJson.devDependencies['@testing-library/jest-dom'] = '^6.1.5'
        packageJson.devDependencies['@vue/vue3-jest'] = '^29.2.6'
      }
    }
    ...
    // 移除 prepare 脚本（模板项目内部使用，生成的项目不需要）
    if (packageJson.scripts && packageJson.scripts.prepare && packageJson.scripts.prepare.includes('prepare-template')) {
    ...
    // 创建额外的配置文件
    if (options.includeHusky) {
      await createHuskyConfig(targetPath, options)
    }
    ...
    if (options.testFramework === 'vitest') {
      await createVitestConfig(targetPath, options.template)
    } else if (options.testFramework === 'jest') {
      await createJestConfig(targetPath, options.template)
    }
 ...
 * 创建 Husky 配置
 */
async function createHuskyConfig(targetPath, options) {
...
`
  ...
  // 根据模板类型配置 lint-staged
  const lintStagedConfig = {}
  if (options.template === 'vue') {
    lintStagedConfig['*.{ts,vue}'] = ['eslint --fix', 'prettier --write']
  } else {
    lintStagedConfig['*.{ts,tsx}'] = ['eslint --fix', 'prettier --write']
  }
  lintStagedConfig['*.{css,json,md}'] = ['prettier --write']
 ...
 * 创建 Vitest 配置
 */
async function createVitestConfig(targetPath, template = 'react') {
  let pluginImport = ''
  let plugins = ''
 ...
 * 创建 Jest 配置
 */
async function createJestConfig(targetPath, template = 'react') {
  let transformConfig = ''
